// cria tag option e adiciona a tag select
...
// objeto com requerimentos dos inputs
let inputs = {
  name: {
    maxLength: 40,
    required: true,
...
function defaultValidation(input, name) {
  let trimmed = input.value.trim(); // verifica tamanho do input value sem espaço em branco
  let validation = inputs[name];  // Exemplo: input[email]

  if(validation.required && trimmed.length === 0) { // verifica se input é vazio
    return false;
  }
...
function dateValidation(input, name){
  ...
  if(!regex.test(input.value)){ // teste vai relacionar e validar a string com o regex
    return {
...
function validateData() {
  let validationsList = {}; // cria objeto vazio

  for(let inputName in inputs) { // percorre objeto do inputs
    let isValid = validateInput(inputName); // chama função
